...
class EditorListInput(discord.ui.Label):
    def __init__(self, editors: Iterable[discord.Member]):
        super().__init__(
            text="編輯者名單",
            component=discord.ui.TextInput(
                style=discord.TextStyle.long,
                default="\n".join(
                    [f"{editor.id}, {editor.display_name}" for editor in editors]
                ),
                required=False,
            ),
        )
...
class ConfigModal(discord.ui.Modal, Cancellable):
    def __init__(self, cfg: ConfigManager, editors: Iterable[discord.Member]):
        ...
        self._editor_list_input = EditorListInput(editors)
    ...
    def channel_id(self) -> int:
        assert isinstance(
            self._remind_channel_dropdown.component, discord.ui.ChannelSelect
        )
